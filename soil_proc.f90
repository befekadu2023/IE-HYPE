...
!------------------------------------------------------------------------
  ...
  SUBROUTINE evaporation_percolation_soilrunoff_linear_reservoir(i,j,temp,epot,wp,fc,epotfrac,soilstate,evap,evapflows,cevap,barefrac, &
    ...
    USE MODVAR, ONLY : basin,soilthick,realzero, &
					   ...
					   timesteps_per_day														!!Added by BTW March1_2022
    USE HYPEVARIABLES, ONLY : basinlp, &
                              m_ttmp,  &
                              m_T1evap, &
                              m_ttrig,m_tredA,m_tredB, &
	...
	USE Jacobian_inv_HYPE			!Added by BTW for the EULER implicit method
	USE Mat_inv_GlobVars									!Added by BTW for the EULER implicit method
	...
	real*8 :: errorx, tol									!Added by BTW for the EULER implicit method
	integer:: iter_count, jk,jkll,kkk									!Added by BTW for the EULER implicit method
	REAL :: epot_daily, infilt_daily
	
	!REAL, DIMENSION(:) :: c_t0(numsubstances)
...
!c_t0 = soilstate%conc(:,1,j,i)	
	
	
		
!Implict backward difference EULER scheme
	
	!write(92000,*)"Implicit Euler selected"
	dim_jacob = 3
	
	!call Allocate_mat_inv_variables(uxx,JacobMatrx,JacobMatrxCopy,		&
    ! &inv_Jacob,row_permutat,matrx_multip,S_new,S_old,S_t0,F_numerator,	&
	! &	dim_jacob)
	
	call Allocate_mat_inv_variables
...
uxx(1) = soilstate%water(1,j,i)
uxx(2) = soilstate%water(2,j,i)
uxx(3) = soilstate%water(3,j,i)

dt = 1.d0/timesteps_per_day		

dim_jacob = SIZE(uxx, DIM = 1)
!dim2_ode = SIZE(uxx,, DIM = 2)
dim2_ode = 1
...
errorx = 1.1*tol 			!Just to initialize the iteration loop


do while (errorx > tol)
!do while (errorx > 0.02)

iter_count  = iter_count +1
!call Jacob_central(S_old,S_t0,JacobMatrx,F_numerator,dt)

epot_daily = 1.0*epot										            !the hourly and daily epot are the same for the preistley-Taylor method
infilt_daily =  1.0*infilt												!the rainfall is converted to daily in the soilmodel0.f90 module to enable maintaining the daily parameters but latter, multipliy with dt to estimate hourly fluxes

!write(39001,*)timesteps_per_day
!write(39002,*)epot_daily
!write(39003,*)infilt_daily



call Jacob_forward_Diff(S_old,S_t0,JacobMatrx,F_numerator,dt,			&
	 &	infilt_daily,barefrac,epot_daily,epotfrac(1),					&
...
!call LU decomposition routine (only once)
  call LUDCMP(JacobMatrx,dim_jacob,row_permutat,CODE_mtrx)

!call solver if previous return code is ok
!to obtain inverse of JacobMatrx one column at a time
  if (CODE_mtrx.eq.0) then
    do jkll=1, dim_jacob
	  call LUBKSB(JacobMatrx,dim_jacob,row_permutat,inv_Jacob(1,jkll))
	  
	end do
  endif
  
  
  call MATMULT(inv_Jacob,F_numerator,matrx_multip,dim_jacob,dim2_ode)
  
  !write(93000,*)inv_Jacob
  
  
  
  S_new = S_old - matrx_multip 
  !write(96000,*)S_new
  !write(96001,*)S_old
  !write(96002,*)S_new - S_old
  
  
  errorx = MAXVAL(dabs(S_new - S_old))
  if(iter_count == 10) errorx=0.9*tol 		!To limit iteration to 10
...
!write(49002,*)S_new(3),
	...
	soilstate%water(1,j,i) = S_new(1)
	soilstate%water(2,j,i) = S_new(2)
	soilstate%water(3,j,i) = S_new(3)
	
	!call deAllocate_mat_inv_variables(uxx,JacobMatrx,JacobMatrxCopy,	&
    ! &inv_Jacob,row_permutat,matrx_multip,S_new,S_old,S_t0,F_numerator)
	
	!Estimate the soil storage1 concentration such that no sink is considered at this stage
	
	soilstate%conc(:,1,j,i) = (soilstate%conc(:,1,j,i)*S_t0(1)+			&
	 &	infilt*cinfilt*dt)/(S_new(1)+dt*(S_old(1)+infilt-S_new(1)))
	...
	perc1= Min(soilpar(m_perc1,isoil)*									&
     ...
     &	Max(0.,(wp(2)+fc(2)+ep(2)) - soilstate%water(2,j,i)))*dt
...
!###############################
	...
	!Estimate the soil storage2 concentration such that no sink is considered at this stage
	
	soilstate%conc(:,2,j,i) = (soilstate%conc(:,2,j,i)*S_t0(2)+			&
	 &	perc1*soilstate%conc(:,1,j,i)*dt)/(S_new(2)+dt*(S_old(2)+perc1-S_new(2)))
	...
	perc2=Min(soilpar(m_perc2,isoil)*									&
     ...
     &	Max(0.,(wp(3)+fc(3)+ep(3)) - soilstate%water(3,j,i)))*dt
...
!###########################
	...
	soilstate%conc(:,3,j,i) = (soilstate%conc(:,3,j,i)*S_t0(3)+			&
	 &	perc2*soilstate%conc(:,2,j,i)*dt)/(S_new(3)+dt*(S_old(3)+perc2-S_new(3)))
	 
	 
	 call deAllocate_mat_inv_variables
