...
MODULE SOILMODEL_DEFAULT
  ...
  USE MODVAR,  ONLY: missing_value,pi,   &
                     ...
                     conduct,simulate, &
                     simulatesubstances, &
					 timesteps_per_day									!!Added by BTW on Mar092022
...
!    REAL trunofftemp(maxsoillayers)
    ...
    !Irrigate the soil     
    IF(conduct%irrigation) CALL apply_irrigation(i,j,epotdist(:,j),pwmm(2,j),soilstate,irrappl,irrsources)
	...
	!write(120042,*)frozenstate%snowdepth(j,i),i,j													!!Added by BTW
    
    !Gross infiltration
    !ginfilt  = rainfall + melt 												!Commented by BTW on Mar092022 to use daily rainfall along with daily parameters even for hourly time step and later use dt = 1/24 to calculate the hourly fluxes
	ginfilt  = timesteps_per_day*rainfall + melt								!Added by BTW on Mar092022 to use daily rainfall along with daily parameters even for hourly time step and later use dt = 1/24 to calculate the hourly fluxes
	!write(13003,*)rainfall,melt,i,j											!!Added by BTW
    ...
    IF(modeloption(p_infiltration)==0 .OR. modeloption(p_infiltration)==1)THEN
      ...
      CALL calculate_infiltration(i,j,isoil,wpmm(:,j),fcmm(:,j),epmm(:,j),ginfilt,cginfilt,temp,mintemp,maxtemp,  &
      ...
      !CALL percolation(i,j,isoil,subid,wpmm(:,j),fcmm(:,j),epmm(:,j),soilthick(:,j),verticalflows(1:2),cverticalflows,soilstate)										!!commented by BTW
	  !CALL percolation_soilrunoff_linear_reservoir(i,j,isoil,subid,wpmm(:,j),fcmm(:,j),epmm(:,j),soilthick(:,j),verticalflows(1:2),cverticalflows,soilstate,soilrunoff,csoilrunoff)			!!Modified by BTW
	  ...
	  runofflows(1:3) = soilrunoff(1:3)					!!Moved by BTW
      crunofflows(:,1:3) = csoilrunoff(:,1:3)				!!Moved by BTW
      crunoff1 = csoilrunoff(:,1)							!!Moved by BTW
      crunoff2 = csoilrunoff(:,2)							!!Moved by BTW
      crunoff3 = csoilrunoff(:,3)							!!Moved by BTW
	ENDIF
	
	

    !Calculate and remove evapotranspiration from the soil upper two layers																									!!moved to the upper if loop by BTW 
    !CALL calculate_actual_soil_evapotranspiration(i,j,temp,epot,wpmm(:,j),fcmm(:,j),epotdist(:,j),soilstate,evap,evapflows(1:2),cevap,MIN(1.,MAX(1.-effsnowcov,0.)))
    ...
    !CALL calculate_soil_runoff(i,j,subid,wpmm(:,j),fcmm(:,j),epmm(:,j), &							!!Moved up by BTW
    !          classdata(j)%streamdepth,soilstate,soilrunoff,csoilrunoff)							!!Moved up by BTW
    !runofflows(1:3) = soilrunoff(1:3)																!!Moved up by BTW
    !crunofflows(:,1:3) = csoilrunoff(:,1:3)														!!Moved up by BTW
    !crunoff1 = csoilrunoff(:,1)																	!!Moved up by BTW
    !crunoff2 = csoilrunoff(:,2)																	!!Moved up by BTW
    !crunoff3 = csoilrunoff(:,3)																	!!Moved up by BTW
    ...
    IF(modeloption(p_infiltration)==2 .OR. modeloption(p_infiltration)==3)THEN
	  !First calculate the evaporation and soil runoff & percolation before infiltration 																	!!Noted by BTW
	  !(percolation came before infiltration (unlike the case for modeloption(p_infiltration)==2 .OR. modeloption(p_infiltration)==3 in the original Hype model) only because it has been integrated with the calculation of soil runoff in the new development					!!Noted by BTW
      !Calculate and remove evapotranspiration from the soil upper two layers																								!!moved here by BTW 
      ...
      !infiltrationflows(3) = excessinfilt
    ENDIF
	!write(13004,*)evap,epot,i,j											!!Added by BTW
    ...
    !IF(modeloption(p_infiltration)==2 .OR. modeloption(p_infiltration)==3)THEN															!!Moved up by BTW because it has been integrated with the calculation of soil runoff
    !  !Percolation down through the soil layers, including DOC-reduction
	...
	!write(120036,*)totalsurfaceflow,excessinfilt,satoverflow,i,j																!!Added by BTW
    ...
    IF(conduct%simP.OR.conduct%simS) CALL particle_processes_for_runoff(i,j,isoil,iluse,dayno,rainfall,totalsurfaceflow,   &
    ...
    !IF(modeloption(p_infiltration)==0 .OR. modeloption(p_infiltration)==1)THEN                                                     !!commented by BTW because it is better to consider the additional volume of water (available from the macropore water flow) to be available for the next time step
    ...
    CALL soil_np_processes(i,j,iluse,conduct%simN,conduct%simP,conduct%simC,dayno,classarea,wpmm(:,j),  &
    ...
    IF(conduct%simC) CALL soil_carbon_processes(i,j,wpmm(:,j),fcmm(:,j),epmm(:,j),pwmm(:,j),soilthick(:,j),    &
    ...
    CALL balance_spsoil(i,j,conduct%simP,soilthick(:,j),soilpar(m_freuc,isoil), &
    ...
    IF(conduct%irrigation) CALL calculate_irrigation_water_demand(i,j,dayno,  &
    ...
    IF(conduct%simC) CALL class_riparian_zone_processes(numsubstances,classheight,runofflows(1)+runofflows(2)+runofflows(3), &
	...
	!write(120041,*)verticalflows(1:2),i,j													!!Added by BTW
